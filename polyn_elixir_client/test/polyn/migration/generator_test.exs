defmodule Polyn.GeneratorTest do
  use ExUnit.Case, async: true

  alias Polyn.Migration.Generator

  @moduletag :tmp_dir

  test "adds folders if they don't exist", %{tmp_dir: tmp_dir} do
    Generator.run(%{name: "foo", dir: tmp_dir})
    assert File.dir?(tmp_dir)
  end

  test "creates a migration", %{tmp_dir: tmp_dir} do
    path = Generator.run(%{name: "my_migration", dir: tmp_dir})
  ...
  test "underscores the filename when generating a migration", %{tmp_dir: tmp_dir} do
    Generator.run(%{name: "MyMigration", dir: tmp_dir})
  ...
  test "raises when existing migration exists", %{tmp_dir: tmp_dir} do
    Generator.run(%{name: "my_migration", dir: tmp_dir})
