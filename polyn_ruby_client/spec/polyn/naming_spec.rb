...
RSpec.describe Polyn::Naming do
  ...
  describe "#validate_message_name!" do
    it "valid name that's alphanumeric and dot separated passes" do
      expect { described_class.validate_message_name!("user.created") }.to_not raise_error
    end

    it "valid name that's alphanumeric and dot separated (3 dots) passes" do
      expect { described_class.validate_message_name!("user.created.foo") }.to_not raise_error
    end

    it "name can't have colons" do
      expect do
        described_class.validate_message_name!("user:test")
      end.to raise_error(Polyn::Errors::ValidationError)
    end

    it "name can't have spaces" do
      expect do
        described_class.validate_message_name!("user   created")
      end.to raise_error(Polyn::Errors::ValidationError)
    end

    it "name can't have tabs" do
      expect { described_class.validate_message_name!("user\tcreated") }
        .to raise_error(Polyn::Errors::ValidationError)
    end

    it "name can't have linebreaks" do
      expect { described_class.validate_message_name!("user\n\rcreated") }
        .to raise_error(Polyn::Errors::ValidationError)
    end

    it "name can't have special characters" do
      expect { described_class.validate_message_name!("user:*%[]<>$!@#-_created") }
        .to raise_error(Polyn::Errors::ValidationError)
    end
    ...
    it "name can't be nil" do
      expect { described_class.validate_message_name!(nil) }
        .to raise_error(Polyn::Errors::ValidationError)
    end
  end
